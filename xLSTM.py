...
class MatrixLSTMCell(nn.Module):
    ...
    def reset_parameters(self):
        ...
        bias_linspace_init(self.fgate.bias, start=3.0, end=6.0)
...
def bias_linspace_init(param: torch.Tensor, start: float = 3.4, end: float = 6.0) -> torch.Tensor:
...
class ViLLayer(nn.Module):
    def __init__(self, dim, direction, expansion=2, qkv_block_size=4, proj_bias=False, conv_bias=True, kernel_size=4):
        super().__init__()
        ...
        self.conv1d = CausalConv1d(dim=inner_dim, kernel_size=kernel_size, bias=conv_bias)
        self.mlstm_cell = MatrixLSTMCell(dim=inner_dim, num_heads=qkv_block_size)
